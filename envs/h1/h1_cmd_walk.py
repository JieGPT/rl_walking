...
class H1CmdWalkEnv(H1BaseEnv):
  ...
  def _build_xml(self) -> str:
    export_dir = self._get_xml_export_dir("h1_cmd_walk")
  ...
  def _setup_obs_normalization(self) -> None:
    self.obs_mean = np.concatenate(
      (
        ...
        self.half_sitting_pose,   # joint pos (10)
        np.zeros(10),             # joint vel
        np.zeros(10),             # joint torque
        [0, 0],                   # gait clock
        [0.25, 0.25, 0.25, 0.25], # mode one-hot (4)
        [0, 0, 0],                # model_ref [yaw, vx, vy]
      )
    )
